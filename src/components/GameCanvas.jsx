...
export default function GameCanvas() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Spawn obstacles
      if (s.frame - s.lastObstacle > 90 + Math.random() * 60) {
        s.obstacles.push({ x: w + 20, h: 20 + Math.random() * 20 });
        s.lastObstacle = s.frame;
      }

      // Spawn coins
      if (s.frame - s.lastCoin > 60 + Math.random() * 40) {
        s.coins.push({ x: w + 20, y: GROUND_Y - 30 - Math.random() * 40 });
        s.lastCoin = s.frame;
      }
